			...
			if (spine_Feet_pair.Num() > i)
			{
				...
				if (SolverReferencePose == ERefPosePluginEnum::VE_Animated)
				{
					...
					//	if (every_foot_dont_have_child == false)
					{
						...
						line_trace_func(*owning_skel, feet_centre_point + f_offset_i + character_direction_vector * (line_trace_upper_height * component_scale) - (right_dir * component_scale * virtual_leg_width), feet_centre_point - character_direction_vector * (line_trace_downward_height * 1 * component_scale) - (right_dir * component_scale * virtual_leg_width), spine_hit_pairs[i].Parent_Right_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, spine_hit_pairs[i].Parent_Right_Hit, FLinearColor::Green, true);
	...
	//	spine_hit_between.AddDefaulted(7);
		...
		for (int32 i = 0; i < spine_Feet_pair.Num(); i++)
		{
			...
			for (int32 j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
			{
				...
				FBoneReference Thigh_Involved = FBoneReference(owning_skel->GetParentBone(Knee_Involved.BoneName));
			...
			if (test_counter < 10)
			{
				...
				spine_hit_pairs[i].RV_SpinalFeet_Hits.AddDefaulted(spine_Feet_pair[i].Associated_Feet.Num());
	...
	if (origin_point_index == 0)
	{
		...
		if (owning_skel->GetOwner())
		{
			...
			if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
				Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z)* Pelvis_SideRotation_Intensity_INPUT*0.5f;
			else
				Intensifier_side = 0;
	...
	else if (origin_point_index == spine_Transform_pairs.Num() - 1)
	{
		...
		if (owning_skel->GetOwner())
		{
			...
			if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || Use_Fake_Chest_Rotations || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
			{
				...
				if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
					right_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z)* Chest_SideRotation_Intensity_INPUT*0.5f;
				else
					right_intensity = 0;
			...
			{
				if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 0)
					right_intensity = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint).Z)*direction* Chest_SideRotation_Intensity_INPUT *-1 * 0.5f;
	...
	if (spine_hit_pairs.Num() > origin_point_index - 1)
	{
			...
			//if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 2)
			{
				...
				*/
				...
				if (origin_point_index == 0)
					...
					Z_Offset = FVector(Z_Offset.X, Z_Offset.Y, FMath::Clamp(Z_Offset.Z, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z - Maximum_Dip_Height_Chest, owning_skel->GetComponentToWorld().InverseTransformPosition(owning_skel->GetComponentLocation()).Z + Maximum_Dip_Height_Chest));
		...
		//	FVector feet_opp_world = owning_skel->GetComponentToWorld().TransformPosition(Feet_Opposite_Mid_Point);
			...
			if (flip_forward_and_right)
			{
				...
				AC_Cross_Value = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z);
			...
			if (atleast_one_hit)
			{
				...
				if (origin_point_index == 0)
				{
					across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z) * Pelvis_Slope_Detection_Strength, 0, 1);

					if (flip_forward_and_right)
					{
						across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z) * Pelvis_Slope_Detection_Strength, 0, 1);
				...
				else
				{
					...
					across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z) * Chest_Slope_Detection_Strength, 0, 1);

					if (flip_forward_and_right)
					{
						across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z) * Chest_Slope_Detection_Strength, 0, 1);
				...
				if (origin_point_index == 0)
				{
					...
					if (Use_Fake_Pelvis_Rotations == false)
					{
						//	new_location += character_direction_vector * (OG_Cross_Value*Slanted_Height_Offset*FMath::Clamp<float>((spine_median_result / 10)*slop_tolerance, 0, 1));
							...
							if (flip_forward_and_right)
							{
								new_location += FVector(0,0,1) * ((-OG_Cross_Value)*Slanted_Height_Offset*FMath::Clamp<float>((spine_median_result / 10)*slop_tolerance, 0, 1));
							}
							else
							{
								new_location += FVector(0, 0, -1) * ((-OG_Cross_Value)*Slanted_Height_Offset*FMath::Clamp<float>((spine_median_result / 10)*slop_tolerance, 0, 1));
					...
					//	new_location = FVector(new_location.X, new_location.Y, FMath::Clamp(new_location.Z, (lerp_data).Z - Maximum_Dip_Height, (lerp_data).Z + Maximum_Dip_Height));
				...
				else
				{
					...
					//	new_location = FVector(new_location.X, new_location.Y, FMath::Clamp(new_location.Z, (lerp_data).Z - Maximum_Dip_Height_Chest, (lerp_data).Z + Maximum_Dip_Height_Chest));
			...
			if (origin_point_index == 0)
			{
				...
				if (owning_skel->GetOwner())
				{
							...
							/*	if (flip_forward_and_right)
								{
									...
									Intensifier_forward = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Left_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Right_Hit.ImpactPoint).Z)* Pelvis_SideRotation_Intensity_INPUT*0.5f;
					...
					else
					{
						...
						float across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z)* Pelvis_Slope_Detection_Strength, 0, 1);

						if (!Pelvis_Flat_Slope_Detection)
							across_ratio = 1;

						Intensifier_forward = FMath::Clamp<float>((Feet_Mid_Point.Z - (Feet_Opposite_Mid_Point.Z)) * Pelvis_ForwardRotation_Intensity_INPUT*across_ratio, -10000, 10000)  *-1;
					...
					if (spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 2 || Use_Fake_Pelvis_Rotations || spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() == 0)
					{
						...
						if (spine_hit_pairs[origin_point_index].Parent_Left_Hit.bBlockingHit&&spine_hit_pairs[origin_point_index].Parent_Right_Hit.bBlockingHit)
						{
							/*if (flip_forward_and_right)
							{
								Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z)* Pelvis_SideRotation_Intensity_INPUT*0.5f;
							}
							else
							*/
							{
					...
					else
					{
						...
						Intensifier_side = (owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[0].ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].RV_Feet_Hits[1].ImpactPoint).Z)*direction* Pelvis_SideRotation_Intensity_INPUT *-1 * 0.5f;
			...
			else if (origin_point_index == spine_Transform_pairs.Num() - 1)
			{
				...
				if (owning_skel->GetOwner())
				{
					...
					else
					{
						...
						float across_ratio = FMath::Clamp<float>(FMath::Abs<float>(owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Front_Hit.ImpactPoint).Z - owning_skel->GetComponentToWorld().InverseTransformPosition(spine_hit_pairs[origin_point_index].Parent_Back_Hit.ImpactPoint).Z)* Chest_Slope_Detection_Strength, 0, 1);

						if (!Chest_Flat_Slope_Detection)
							across_ratio = 1;


						forward_intensity = FMath::Clamp<float>(((Feet_Opposite_Mid_Point.Z) - Feet_Mid_Point.Z) * Chest_ForwardRotation_Intensity_INPUT* across_ratio, -10000, 10000)  *-1;
...
//				final_transformation.SetLocation(pitch_based_location);
			...
			//	final_transformation.SetLocation(spine_hit_pairs[origin_point_index].Parent_Spine_Hit.ImpactPoint);
				...
				if (origin_point_index == 0)
					final_transformation.SetLocation(FVector(final_transformation.GetLocation().X, final_transformation.GetLocation().Y, FMath::Clamp(final_transformation.GetLocation().Z, (owning_skel->GetComponentToWorld().TransformPosition(lerp_data)).Z - Maximum_Dip_Height, owning_skel->GetComponentToWorld().TransformPosition(lerp_data).Z + Maximum_Dip_Height)));
				else
					final_transformation.SetLocation(FVector(final_transformation.GetLocation().X, final_transformation.GetLocation().Y, FMath::Clamp(final_transformation.GetLocation().Z, (owning_skel->GetComponentToWorld().TransformPosition(lerp_data)).Z - Maximum_Dip_Height_Chest, owning_skel->GetComponentToWorld().TransformPosition(lerp_data).Z + Maximum_Dip_Height_Chest)));
...
DragonSpineOutput FAnimNode_DragonSpineSolver::DragonSpineProcessor_Direct(FTransform& EffectorTransform, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	else*/
	{
		...
		//	else
		{
			...
			float Max_Range_Limit = FMath::Clamp<float>( (CSEffectorLocation - Chain[0].Position).Size()/ MaximumReach, min_extension_ratio, max_extension_ratio);
			...
			CSEffectorLocation = Chain[0].Position + (CSEffectorLocation - Chain[0].Position).GetSafeNormal()* MaximumReach * Max_Range_Limit_Lerp;
		}
